...
private:
    ...
    typedef struct context {
        ...
        // Events associated with this coroutine, as reported by epoll_wait
        uint32_t events = 0;
    } context;
...
public:
    ...
    /**
     * Get events associated with current coroutine
     * Events are set, for example, by _block_on_epoll
     */
    uint32_t get_events() const;

    /**
     * Events setter + Wake up coroutine
     * See comments above definition for the reasoning
     * (long story short: context is private in Engine)
     */
    void setEventsWake(uint32_t events, void *ptr);
     ...
     */
    void start(std::function<void()> idle_func, std::function<void()> main) {
    ...
    void *run(std::function<void()> func) {
        ...
        pc->next = alive;
        pc->blocked = false; // TODO: спросить - нужна ли эта инициализация,
        pc->events = 0;      // или оно само из определения структуры?
        alive = pc;
        if (pc->next != nullptr) {
            pc->next->prev = pc;
        }
